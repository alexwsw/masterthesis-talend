...
public class tMap extends AbstractNode {
	...
	public static Element newInstance(Document document, Document template,
		...
		Node n = XPathFinder.findNode(template,
				XPathExpressions.GETBYCOMPONENTSNAME, componentName);
		// true = all child elements are copied as well
		Element copy = (Element) document.importNode(n, true);
		Element label = (Element) XPathFinder.findNode(copy,
				XPathExpressions.GETNAMEATTRIBUTE, "LABEL");
		label.setAttribute("value", name);
		Element uniqueName = (Element) XPathFinder.findNode(copy,
	...
	public static void resetNode(Document document, Node input)
		...
		NodeList mData = XPathFinder.findNodeList(input,
	...
	public static Node getNodeData(Node node) throws WrongNodeException {
		...
		if (!(AbstractNode.verifyNodeType(node).equals(componentName))) {
			...
			Node a = XPathFinder.findNode(node,
	...
	public static Element createVarTables(Document document, Node node) throws WrongNodeException {
		...
		return tables;
			...
			return (Element) XPathFinder.findNode(node, XPathExpressions.FINDELEMENT, "varTables", "Var");
		}
	}
	...
	public static Element createOutputTables(Document document, Node node, String name) throws WrongNodeException {
		...
		return tables;
			...
			return (Element) XPathFinder.findNode(node, XPathExpressions.FINDELEMENT, "outputTables", name);
		}
	}
	...
	public static Element createInputTables(Document document, Node node, String connLabel) throws WrongNodeException {
		...
		return inputTables;	
			...
			return (Element) XPathFinder.findNode(node, XPathExpressions.FINDELEMENT, "inputTables", connLabel);
		}
	}
	...
	public static Element createTMapMetadata(Document document, Node outputTables) {
		...
		return metaData;
			...
			return (Element) XPathFinder.findNode(outputTables.getParentNode().getParentNode(),XPathExpressions.FINDELEMENT, "metadata", output.getAttribute("name"));
		}
	}
	...
	public static void doLookup (Document document, Document template, LookupObject data) throws WrongNodeException {
		...
		Element startMetadata = (Element) XPathFinder.findNode(document, XPathExpressions.getMetaDataForConnection, startConnection.getAttribute("metaname"));
	...
	public static boolean doesElementExist (Node node, String elementTag, String parameter) {
		Node result = XPathFinder.findNode(node, XPathExpressions.FINDELEMENT, elementTag, parameter);
		if(result == null) {
	...
	public static Element getElement (Node tables, String name){
		return (Element) XPathFinder.findNode(tables, XPathExpressions.FINDATTRIBUTE, name);
	}
