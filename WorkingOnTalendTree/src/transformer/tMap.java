...
public class tMap extends AbstractNode {
	...
	public static void resetNode(Document document, Node input) {
		...
		/*
		 * Node child = mData.item(0).getFirstChild(); while
		 * (child.getNodeType() == Node.TEXT_NODE) { child =
		 * child.getNextSibling(); } Element dummy = (Element)
		 * child.cloneNode(true); dummy.setAttribute("name", "dummy"); for (int
		 * i = 0; i < mData.getLength(); i++) {
		 * NodeBuilder.removeAllChildNodes(mData.item(i));
		 * NodeBuilder.appendElementToContext(mData.item(i), (Element)
		 * dummy.cloneNode(true)); } child = null; dummy = null; // reset
		 * nodeData Node nodeData = getNodeData(input); NodeList inputTables =
		 * Navigator.processXpathQueryNodeList(nodeData,
		 * XPathExpressions.getTMapInputTables, null); Node varTables =
		 * Navigator.processXPathQueryNode(nodeData,
		 * XPathExpressions.getByNodeName, "varTables"); NodeList outputTables =
		 * Navigator.processXpathQueryNodeList(nodeData,
		 * XPathExpressions.getTMapOutputTables, null); child =
		 * inputTables.item(0).getFirstChild(); while (child.getNodeType() ==
		 * Node.TEXT_NODE) { child = child.getNextSibling(); } dummy = (Element)
		 * child.cloneNode(true); NodeBuilder.removeAllChildNodes(varTables);
		 * NodeBuilder.appendElementToContext(varTables, (Element)
		 * dummy.cloneNode(true)); for (int i = 0; i < inputTables.getLength();
		 * i++) { NodeBuilder.removeAllChildNodes(inputTables.item(i));
		 * NodeBuilder.appendElementToContext(inputTables.item(i), (Element)
		 * dummy.cloneNode(true)); } for (int i = 0; i <
		 * outputTables.getLength(); i++) {
		 * NodeBuilder.removeAllChildNodes(outputTables.item(i));
		 * NodeBuilder.appendElementToContext(outputTables.item(i), (Element)
		 * dummy.cloneNode(true)); }
		 */
	...
	public static String setPrefix(Document document, Node nodeData,
			LookupObject data) throws WrongNodeException {
	...
	// in progress
	public static String setInputTables(Document document, Node node,
			Collection<ColumnObject> columns, EConnectionTypes type) {

		Element incomingConnection = (Element) Connection.findConnection(
				document, node, type);
		Element inputTables = tMap.createInputTables(document, node,
				incomingConnection.getAttribute("label"));
	...
	public static String setInputTables(Document document, Node node,
			Collection<ColumnObject> columns, LookupObject data,
			String mainInputTables, EConnectionTypes type) {
		...
		if (data != null) {
			if (data instanceof Lookup2Object) {
				...
				String expression = String
						.format("%s.%s > %s.%s && %s.%s %s %s.%s",
								mainInputTables, temp.getLU2ValidParameter(),
								inputTables.getAttribute("name"),
								temp.getLU2FromColumn(), mainInputTables,
								temp.getLU2ValidParameter(), upperBound,
								inputTables.getAttribute("name"),
								temp.getLU2ToColumn());
			...
			for (ColumnObject column : columns) {
				...
				if (column.getName().equals(data.getLookupColumn())) {
					...
					dummy.setAttribute(
							"expression",
							tMap.setJoinForLookup(mainInputTables,
									inputTables.getAttribute("name"), data));
				}
				dummy.setAttribute("name", column.getName());
				dummy.setAttribute("type", column.getType());
				NodeBuilder.appendElementToContext(inputTables, dummy);
			...
			for (ColumnObject column : columns) {
				if (columnExists(column.getName(), inputTables)) {
					continue;
				}
				Element dummy = tMap.createNodeDataColumnDummy(document);
				dummy.setAttribute("name", column.getName());
	...
	public static Element createVarTables(Document document, Node node)
		...
		if (!(tMap.doesElementExist(node, "varTables", "Var"))) {
			...
			return (Element) Navigator.processXPathQueryNode(node,
					XPathExpressions.findElement, "varTables", "Var");
		}
	}

	public static Element createOutputTables(Document document, Node node,
			String name) {
	...
	public static Element createInputTables(Document document, Node node,
			String connLabel) {
		if (!(tMap.doesElementExist(node, "inputTables", connLabel))) {
			...
			return (Element) Navigator.processXPathQueryNode(node,
					XPathExpressions.findElement, "inputTables", connLabel);
		}
	}

	public static Element createTMapMetadata(Document document,
			Node outputTables) {
		Element output = (Element) outputTables;
		if (!(tMap.doesElementExist(outputTables.getParentNode()
				.getParentNode(), "metadata", output.getAttribute("name")))) {
			Element metaData = null;
			String tableName = output.getAttribute("name");
			// Element dummy =
			// (Element)AbstractNode.createMetadataColumnDummy(document);
			metaData = document.createElementNS("http://www.talend.org/mapper",
					"metadata");
			...
			return (Element) Navigator.processXPathQueryNode(outputTables
					.getParentNode().getParentNode(),
					XPathExpressions.findElement, "metadata", output
							.getAttribute("name"));
		}
	}

	// perhaps not necessary
	public static void setTablesFromDTO(Document document, Node tables,
			Collection<ColumnObject> columns) {
		for (ColumnObject column : columns) {
			Element dummy = tMap.createNodeDataColumnDummy(document);
			dummy.setAttribute("name", column.getName());
			dummy.setAttribute("type", column.getType());
	...
	public static Element setLookupOutput(Document document, Node node,
			String name, Collection<ColumnObject> inputColumns,
			Collection<ColumnObject> lookupColumns, LookupObject data,
			String mainTable, String secondaryTable) {
		...
		for (ColumnObject column : inputColumns) {
			...
			Element dummy = tMap.createNodeDataColumnDummy(document);
			tMap.addAttribute(document, dummy);
			dummy.setAttribute("expression",
					String.format("%s.%s", mainTable, column.getName()));
			dummy.setAttribute("name", column.getName());
			dummy.setAttribute("type", column.getType());
			...
			for (Map.Entry<String, String> entry : data
				...
				if (column.getName().equals(entry.getKey())) {
					...
					dummy.setAttribute("expression", String.format(
							"(%s.%s == %s)? %s : %s.%s", secondaryTable,
							entry.getKey(), setNullHandling(column),
							setDefault(column.getType()), secondaryTable,
							entry.getKey()));
					dummy.setAttribute("name", entry.getValue());
					dummy.setAttribute("type", column.getType());
					NodeBuilder.appendElementToContext(outputTables, dummy);
					ColumnObject returnObject = getObjectByName(
							data.getPackageReturnColumns(), entry.getValue());
					Element metadataDummy = AbstractNode
							.setMetadataColumnFromDTO(document, returnObject,
									metaData);
		...
		ColumnObject object = getObjectByName(lookupColumns,
				data.getLookupColumn());
		Element dummy = tMap.createNodeDataColumnDummy(document);
		dummy.setAttribute("expression", String.format(
				"(%s.%s == null)? ++%s.%s : %s.%s", secondaryTable,
				data.getLookupColumn(), mainTable, "isETL_ErrorCount",
				mainTable, "isETL_ErrorCount"));
	...
	public static Element setOutput(Document document, Node node, String name,
			Collection<ColumnObject> inputColumns, LookupObject data,
			String mainTable) {
		Element outputTables = tMap.createOutputTables(document, node, name);
		Element metaData = tMap.createTMapMetadata(document, outputTables);
		if (data != null) {
			...
			if (data.getPrefix() != null) {
				matchColumn = String.format("String.valueOf(%s ",
						data.getPrefix());
				if (data.getPackageColumns() != null) {
					for (String column : data.getPackageColumns()) {
						matchColumn = matchColumn
								+ String.format(" + %s.%s", mainTable, column);
					}
				}
			} else {
				matchColumn = "String.valueOf(";
				if (data.getPackageColumns().get(0) != null) {
					for (String column : data.getPackageColumns()) {
						matchColumn = matchColumn
								+ String.format("%s.%s + ", mainTable, column);
					}
					matchColumn = matchColumn.substring(0,
							matchColumn.length() - 3);
				} else {
					matchColumn = matchColumn + String.format("%s.%s", mainTable, data.getLookupColumn());
				}
			}
			matchColumn = matchColumn + ")";
			Element nodeDummy = tMap.createNodeDataColumnDummy(document);
			tMap.addAttribute(document, nodeDummy);
			nodeDummy.setAttribute("name",
					data.getPackageOutputColumn_MatchColumn());
			// probably get it from the Lookup table
			ColumnObject lookupColumn = tMap.getColumnFromDTO(
					data.getLookupTableColumns(), data.getLookupColumn());
			...
			AbstractNode.setMetadataColumnFromDTO(document, lookupColumn,
					metaData);
		}
		for (ColumnObject column : inputColumns) {
			System.err.println(column.toString());
			if (columnExists(column.getName(), outputTables)) {
				continue;
			}
			Element dummy = tMap.createNodeDataColumnDummy(document);
			tMap.addAttribute(document, dummy);
	...
	public static ColumnObject getColumnFromDTO(
			Collection<ColumnObject> columns, String name) {
		for (ColumnObject column : columns) {
	...
	public static Node doLookup(Document document, Document template,
			LookupObject data) {
		// test random Number for lookup Name
		Random r = new Random();
		// ConnectionPoint mark must be changed connection(Label), inputTables
		// and metadata (tMap)
		// setting/removing of ConnectionPoint should be outsourced in a
		// separate method
		// ConnectionPoint as unique_name? could be more useful....
		String startPointMark = "ConnectionPoint";
		Element startConnection = (Element) AbstractNode.getElementByValue(
				document, startPointMark);
		Element startMetadata = (Element) Navigator.processXPathQueryNode(
				document, XPathExpressions.getMetaDataForConnection,
				startConnection.getAttribute("metaname"));
		Element previousNode = (Element) startMetadata.getParentNode();
		// relabel the connection (essential for the inputTables)
		AbstractNode.setAttribute(startConnection, "UNIQUE_NAME",
				startConnection.getAttribute("metaname"));
		// startConnection.setAttribute("label",
		// startMetadata.getAttribute("name"));
		Element prefixTMap = tMap
				.newInstance(document, template, "PrefixMaker");
		Element lookupTMap = tMap.newInstance(document, template, "LookupNode");
		Element lookupDb = tMSSqlInput.newInstance(document, template,
				"LookupDB", data.getLookupTableColumns(), "Henry DWH",
				data.getLookupTable());
		Connection.newConnection(document, template,
				AbstractNode.getMetadata(document, lookupDb, "FLOW"),
				lookupTMap, EConnectionTypes.Lookup);
		// redirect the startConnection to the prefix tMap Node
		startConnection.setAttribute("target",
				AbstractNode.getNodesUniqueName(document, prefixTMap));
		// put the entire data from the input Dto into the inputTables
		// Element inputTables = tMap.createInputTables(document, prefixTMap,
		// startConnection.getAttribute("label"));
		String inputName = tMap.setInputTables(document, prefixTMap,
				tMap.extractMetadata(startMetadata), EConnectionTypes.Main);
		Element prefixMData = tMap.setOutput(document, prefixTMap,
				"meinOutput", tMap.extractMetadata(startMetadata), data,
				inputName);
		Connection.newConnection(document, template, prefixMData, lookupTMap,
				EConnectionTypes.Main);
		String nameInputTable = tMap.setInputTables(document, lookupTMap,
				tMap.extractMetadata(prefixMData), EConnectionTypes.Main);
		String nameLookupTable = tMap.setInputTables(document, lookupTMap, tMap
				.extractMetadata(AbstractNode.getMetadata(document, lookupDb)),
				data, nameInputTable, EConnectionTypes.Lookup);
		Element lookupMetadata = tMap.setLookupOutput(document, lookupTMap,
				("Lookup" + r.nextInt(100000)), tMap
						.extractMetadata(prefixMData), tMap
						.extractMetadata(AbstractNode.getMetadata(document,
								lookupDb)), data, nameInputTable,
				nameLookupTable);
		Element newConnection = Connection.newConnection(document, template,
				lookupMetadata,
				AbstractNode.getElementByValue(document, "Calculations"),
				EConnectionTypes.Main);
		AbstractNode.setAttribute(newConnection, "UNIQUE_NAME",
				"ConnectionPoint");
		/*
		 * Element outputTables = tMap.createOutputTables(document,
		 * "preparedOutput"); Element outputMData =
		 * tMap.createMetadata(document, outputTables);
		 */

		// String varTableName = tMap.setPrefix(document,
		// tMap.getNodeData(prefixTMap), data);

		// put nodes onto the proper place in the designer (separate method?)
		prefixTMap.setAttribute("posY", String.valueOf(Integer
				.parseInt(previousNode.getAttribute("posY")) + 150));
		System.err.printf("Previous Node: %s%n",
				AbstractNode.getNodesUniqueName(document, previousNode));
		System.err.printf("Node: %s, PosX = %s, PosY = %s",
				AbstractNode.getNodesUniqueName(document, prefixTMap),
				prefixTMap.getAttribute("posX"),
				prefixTMap.getAttribute("posY"));
		lookupTMap.setAttribute("posY", String.valueOf(Integer
				.parseInt(prefixTMap.getAttribute("posY")) + 150));
		System.err.printf("Node: %s, PosX = %s, PosY = %s",
				AbstractNode.getNodesUniqueName(document, lookupTMap),
				lookupTMap.getAttribute("posX"),
				lookupTMap.getAttribute("posY"));
		lookupDb.setAttribute("posX", String.valueOf(Integer
				.parseInt(prefixTMap.getAttribute("posX")) + 150));
		lookupDb.setAttribute("posY", lookupTMap.getAttribute("posY"));
		System.err.printf("Node: %s, PosX = %s, PosY = %s",
				AbstractNode.getNodesUniqueName(document, lookupDb),
				lookupDb.getAttribute("posX"), lookupDb.getAttribute("posY"));
	...
	public static boolean doesElementExist(Node node, String elementTag,
			String parameter) {
		Node result = Navigator.processXPathQueryNode(node,
				XPathExpressions.findElement, elementTag, parameter);
		if (result == null) {
	...
	public static String setJoinForLookup(String inputTableName,
			String lookupTableName, LookupObject data) {
		return String.format("%s.%s", inputTableName,
				data.getPackageOutputColumn_MatchColumn());
	}

	public static Element getElement(Node tables, String name) {
		return (Element) Navigator.processXPathQueryNode(tables,
				XPathExpressions.findAttribute, name);
	}

	public static void setPrefixMakerNode(Node prefixTMap, LookupObject data,
			Collection<ColumnObject> packageColumns) {

	}

	// if there's a collection of multiple Lookups (needs to be tested!!!)
	public static Node doLookup(Document document, Document template,
			Collection<LookupObject> data) {
		Node node = null;
	...
	public static ColumnObject getObjectByName(
			Collection<ColumnObject> columns, String name) {
	...
	public static void addTechnicalCols(Document document, SourceObjectDTO s) {
		...
		Node technicalFields = AbstractNode.getElementByValue(document,
				"DER isPKNULL und DER technical fields");
		Node nodeDataTechnicalFields = tMap.getNodeData(technicalFields);
		Node functionalFields = AbstractNode.getElementByValue(document,
				"DER FUNCTIONAL FIELDS");
		Node nodeDataFunctionalFields = tMap.getNodeData(functionalFields);
		Element inputTables = (Element) Navigator.processXPathQueryNode(
				nodeDataTechnicalFields, XPathExpressions.getInputTables, null);
		String inputTablesName = inputTables.getAttribute("name");
		Node varTables = Navigator.processXPathQueryNode(
				nodeDataFunctionalFields, XPathExpressions.getVarTables, null);
		Node outputTables = Navigator
				.processXPathQueryNode(nodeDataTechnicalFields,
						XPathExpressions.getOutputTables, null);
		// expression for the isETL_isValid & isETL_isPKNULL values
		String expression = String.format("(%s.%s == null ", inputTablesName,
				splitPk[0].trim());
		// expression for the BK variable
		String expressionPk = String
				.format("String.valueOf(globalMap.get(\"FELC1_TargetfieldValue\"))+\"-\"+%s.%s",
						inputTablesName, splitPk[0].trim());
		for (int i = 1; i < splitPk.length; i++) {
			expression = expression
					+ String.format("|| %s.%s == null", inputTablesName,
							splitPk[i].trim());
			expressionPk = expressionPk
					+ String.format("+\"-\"+%s.%s", inputTablesName,
							splitPk[i].trim());
		}
		// expressionPk = expressionPk + ")";
		String expressionIsValid = expression
				+ String.format(") ? false : %s.isETL_isValid", inputTablesName);
		String expressionIsPkNull = expression + ") ? true : false ";
		// expression for the Name variable
		String expressionName = String.format("%s.%s", inputTablesName,
				splitName[0].trim());
		...
		// expressionName = expressionName + ")";
		Element isETL_isValid = (Element) Navigator.processXPathQueryNode(
				outputTables, XPathExpressions.getByNameAttribute,
				"isETL_isValid");
		isETL_isValid.setAttribute("expression", expressionIsValid);
		Element isETL_isPKNULL = (Element) Navigator.processXPathQueryNode(
				outputTables, XPathExpressions.getByNameAttribute,
				"isETL_isPKNULL");
		isETL_isPKNULL.setAttribute("expression", expressionIsPkNull);
		Element BK = (Element) Navigator.processXPathQueryNode(varTables,
				XPathExpressions.getByNameAttribute, "BK");
		BK.setAttribute("expression", expressionPk);
		Element name = (Element) Navigator.processXPathQueryNode(varTables,
				XPathExpressions.getByNameAttribute, "Name");
		name.setAttribute("expression", expressionName);
	}

	public static boolean columnExists(String columnName, Node node) {
	...
	public static Node getTMapMetadata(Document document, Node node,
			String nameMetadata) {
